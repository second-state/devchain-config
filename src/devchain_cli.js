...
const argv = require('yargs')
        ...
        .string('params.foundation_address')
        .argv;
...
if (argv.genesis === undefined){
    ...
    jsonContent = jsonfile.readFileSync(argv.genesis)
}
...
if (argv.validators !== undefined) {
    ...
    for (let key in validator_params) {
        if (key in jsonContent.validators[0]) {
            jsonContent.validators[0][key] = validator_params[key]
        }
      }
}

if (argv.params !== undefined) {
    // for (let key in argv.params) {
    //     if (key in jsonContent.params) {
    //         jsonContent.params[key] = argv.params[key]
    //     }
    //   }
